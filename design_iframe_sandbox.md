...
**実装状況: 実装完了（v2.23.0）。iOS 実機の描画確認まで済み。**

> ⚠ **その後 v2.24.0 で `allow-same-origin` を与える形に改めた。** 不透明オリジンのフレームには
> **Chrome / Edge のページ翻訳が届かず、本文だけ原文のまま残る**ことが実測で分かったため
> （想定利用者は日本語学習者）。「ePub のコードを動かさない」役目は **nonce つき CSP** に移した。
> **この文書の §3-1「`allow-same-origin` は与えない」は現在の設計ではない。**
> 現行は `design_browser_translation.md` を見ること。ローカルフォントの data URI 一本化（§3-2）と
> §6 の `reflowKeepPos()` はそのまま有効。
実装中に**初期からの別のバグ**（サイドバー開閉で本文がずれる）を見つけて同時に直した（§6）。
...
Step 1〜3 は完了。Step 4 は **iOS の描画確認まで良好**（2026-08-31）。
...
## 6. 実装中に見つかった別のバグ（同時修正）

**症状**: iOS で目次から章へ飛んだあとサイドバーを閉じると、**本文が左にずれて右にサイドバー幅ぶんの余白**が出る。

**sandbox 化による退行ではない。** v2.21.0 にも同じ問題があり、`resize` の出現数も当時と同じ 2
（＝**iframe 側に元からハンドラが無い**）。実装中の実機確認でたまたま見つかった初期からの不具合。

**原因**: transform スクロールでは位置が viewport 寸法に依存する。

```js
function maxS(){ return Math.max(0, body.offsetWidth - vw()); }
setTx(-ms);   // 'start' = 本文の右端を viewport 右端に合わせる（縦書きは右端が読み始め）
```

サイドバーが閉じて `vw` が広がると `ms` が縮むので、**同じ読書位置を保つには `tx` を `+Δ` 右へ
動かす必要がある**。誰も動かしていなかったので本文が Δ ぶん左に残っていた。

**対処**: 読書位置を**比率**で `_ratio` に持ち（更新点は `setTx`/`setTy` の中だけ）、`resize` から
150ms のデバウンスで `reflowKeepPos()` が新しい `ms` から引き直す。寸法が変わっていなければ何もしない。

- **3 つの IIFE すべてに要る**。**比率の向きが軸で逆**（縦書きは `0=先頭` が `tx=-ms`、
  横書きは `0=先頭` が `ty=0`）なので、コピペで済ませない
- **`yomikake.html` には入れない**。本物のスクロール API はブラウザが位置を保つ。
  transform スクロールは iOS 専用の機構であり、この補正もそれに属する
- テストは `tests/cases/scroll-reflow.js`（本体 11 / iOS 26 assertion）。
  iOS 側に入っていることと**本体側に入っていないこと**の両方を見る

**副産物**: `tests/lib/syntax-check.js` は HTML 内の `<script>` しか見ておらず、
**iframe へ注入される文字列は素通りしていた**。`scroll-reflow.js` が 3 モードとも
`new Function(src)` で構文を確かめるようにして、その穴を塞いだ。
