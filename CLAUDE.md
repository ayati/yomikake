...
- **⚠ `share_target` の `accept` にはワイルドカード（`application/*` と `*/*`）を必ず残す。** Chrome は共有された MIME が accept に当たらないと**パートを作る前にファイルを落とす**（body は 終端行だけの空 multipart になる）。Android のファイラーは `.epub` を `*/*` で投げることが多い。`tests/cases/share-receive.js` が検査している。**manifest の変更は WebAPK に焼かれるので、実機では入れ直さないと反映されない。**
