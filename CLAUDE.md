...
- ePub 由来の**インライン `on*` ハンドラ・入れ子の `<iframe>`/`<object>`/`<embed>`・`javascript:` スキーム**も `buildSrcdoc()` で除去する（v2.22.1）。**v2.22.0 までは「リスクが低く除去コストが高い」として意図的に残していたが、KOReader 同期で localStorage に長期の資格情報（`userkey` はパスワードと等価）が載ったので方針を改めた。** srcdoc の iframe は親と同一オリジンで `sandbox` も無いため、ePub でコードが動くと `localStorage` を丸ごと読める。`<script>` を先に消してあるので、残っていた `onload` 等は参照先を失った死にコードであり、除去して壊れる正当な ePub は無い。
  ...
  - ⚠ これは数え上げ型の防御で、**本命は v2.23.0 で入れた iframe の sandbox**（下記）。両方を残すのは多層防御。

**本文 iframe の sandbox 化（v2.23.0）** (`<iframe id="content-iframe" sandbox="allow-scripts allow-popups allow-popups-to-escape-sandbox">`, `cfGetFontSrc`, `cfRegisterPreviewFace` — 設計書 `design_iframe_sandbox.md`)。

- **`allow-same-origin` は絶対に与えない。** 与えると sandbox は自分で自分を外せるので無意味になる。これで ePub 由来のコードが動いても**不透明オリジン**になり、親の `localStorage`（KOReader 同期の資格情報・しおり・読書データ）へ到達できない。v2.22.1 の除去（`on*` / 入れ子 / `javascript:`）は数え上げ型なので、こちらが本命。両方残して多層防御にしてある。
...
- テストは `tests/cases/epub-sanitize.js`（両ファイル各 33 assertion）。`allow-same-origin` が無いこと・親から `contentDocument` を覗けないこと・`createObjectURL` が消えたことを検査している。**⚠ タイミング由来の退行は自動テストで担保できない**（headless は rAF を差し替えている）ので実機確認が要る。
...
- ePub 由来の**インライン `on*` ハンドラ・入れ子の `<iframe>`/`<object>`/`<embed>`・`javascript:` スキーム**も `buildSrcdoc()` で除去する（v2.22.1）。**v2.22.0 までは「リスクが低く除去コストが高い」として意図的に残していたが、KOReader 同期で localStorage に長期の資格情報（`userkey` はパスワードと等価）が載ったので方針を改めた。** srcdoc の iframe は親と同一オリジンで `sandbox` も無いため、ePub でコードが動くと `localStorage` を丸ごと読める。`<script>` を先に消してあるので、残っていた `onload` 等は参照先を失った死にコードであり、除去して壊れる正当な ePub は無い。
  ...
  - `javascript:` は `<a href>` なら `CLICK_HANDLER` → `handleIframeLink()` が既に拒否していたが、**SVG の `<a xlink:href>` は `getAttribute('href')` が null になり `preventDefault` されず素通りしていた**。属性値そのものを見て落とす。
  - ⚠ これは数え上げ型の防御で、**本命は v2.23.0 で入れた iframe の sandbox**（下記）。両方を残すのは多層防御。
  - ⚠⚠ **ただし sandbox 化には既知の障害が 1 つある: ローカルフォント（`custom:`）。** `cfGetFontSrc()` は http(s) で **`blob:` URL** を返し、`buildSrcdoc()` がそれを `@font-face{src:url(...)}` として iframe に埋め込む。**blob: URL は生成元オリジンに紐づくので、不透明オリジンの iframe からは読めない**（**iPad Safari 実機で `blob fetch: 失敗 / Load failed` を確認済み・2026-08-30**。WebKit には bug 170075「Cannot read blobs in sandboxed iframes」があり 2021 年に RESOLVED FIXED だが、直後に regression 222312 が立っている＝**歴史的に弱く、現に今も落ちる**）。回避は `cfGetFontSrc()` の `file://` 側と同じ **data URI 経路を常用**することだが、実装のコメントどおり**27MB 級の文字列**になり、章を描くたびに srcdoc へ埋め込むことになる。iPad では実用にならない可能性が高い。**sandbox 化に着手するなら、この一点の設計が先。**
  - **同じ実機検査で、sandbox 自体は防御として機能することも確認済み**（iframe 内で `origin = null` / `localStorage: SecurityError`）。`postMessage` / `getComputedStyle` / `PointerEvent` / `ontouchstart` はいずれも生きており、**iOS の transform スクロール機構は動く見込み**。クロスオリジン fetch（fonts.gstatic.com）も CORS で弾かれない。つまり**残る障害はローカルフォント 1 点だけ**。
  - 有力な回避案は **iframe 側で `new FontFace(name, arrayBuffer)` ＋ `document.fonts.add()`**（blob も data URI も要らない）。ただし iframe は章ごとに作り直されるため、**章を描くたびにフォント実体を `postMessage` で送る**必要があり、そのコストは未評価。検査ページは `temp_sample/sandbox probe.png` に結果あり。
