...
**PWA assets (v2.6.0):** the "two-file" model now ships **7 companion files in the same `/book/` directory** (still no build step): `manifest.webmanifest` (+ `manifest_ios.webmanifest`), `sw.js`, and 4 PNG icons (`icon-192`, `icon-512`, `icon-512-maskable`, `apple-touch-icon`). All PWA behavior is progressive-enhancement — `file://`, uninstalled tabs, and SW-unsupported browsers behave exactly as before. `sw.js` does two things: (1) **Web Share Target** — a `POST .../share-receive` (declared in `manifest.webmanifest`'s `share_target`, Android Chrome only) has its `epub` file stashed in a dedicated IDB (`epub_viewer_share` / store `pending` / key `'file'`, **separate from the `epub_viewer_files` cache DB**) and redirects to `yomikake.html?shared=1`; the page's Init `handleSharedFile()` reads it via `_shareTake()`, applies a 10-min freshness guard, and calls `loadEpub()`（受け渡しの詳細は下の §共有ターゲットの受け取り）. (2) **App-shell offline** — HTML navigation is **network-first** (updates land immediately; cache only on failure), static assets cache-first. **On release, bump `VERSION` in `sw.js`** when HTML changes. iOS gets manifest+SW registration only (no share receiver — iOS Safari has no Web Share Target); its home-screen install uses a **separate storage bucket** from Safari (bookmarks migrate via Drive; ePub cache / local fonts need re-open). `loadEpub()` now validates the ZIP magic bytes (`PK`) up front and shows `toast.sharedNotEpub` for non-ePub input (octet-stream shares / drag-drop) — in **both files**.

**共有ターゲットの受け取り（2026-09 に作り直し・`yomikake.html` のみ）** (`shareExtractFile`, `shareParseMultipart`, `shareStash`, `shareTimeout`, `shareRedirect`, `errName`（以上 `sw.js`）, `_SHARE_CACHE`, `_SHARE_STASH_PATH`, `_SHARE_FRESH_MS`, `_shareIdbPeek`, `_shareCachePeek`, `_shareTake`, `_shareClear`, `_shareOpen`, `_sharedPendingFile`, `showShareFailToast`)。

- **発端**: Android 実機（Chrome 153）でファイラーからの共有が `toast.shareFailed` で失敗するようになった。**共有経路のコードは v2.6.0 から無変更**で、手元（Chrome 145）では実ファイル・実ファイル名・ナビゲーション POST で完走する。つまり退行ではなく端末側の事情だが、**失敗点が 3 箇所あって全部が同じ文言を出すため、実機から情報を持ち帰れなかった**。これが本当の不具合。
- **必ず理由コードを返す。** `sw.js` は `?shared=err&r=<理由>` で返し（`form:<例外名>` / `nofile:empty` / `nofile:absent` / `raw:nopart` / `idb:<名>/cache:<名>` / `*:TimeoutError`）、さらに**届いた中身の要約**（`keys=`＝フォームのパート一覧・`len=`＝生 body のバイト長・`ct=`）を添える。ページ側は `empty`（退避に何も無い）/ `stale`（鮮度切れ）/ `load:<例外名>` を足してトーストに出す。**「原因不明のまま握りつぶす」を新たに作らないこと。**
- **⚠ accept を広げても直らない（実測で決着）。** `*/*` / `application/*` を焼いた WebAPK （パッケージ名が変わったことで新規インストールを確認）でも body は空の multipart のままだった。**この道は塞がっている**ので、共有シートを賑やかにするだけのワイルドカードは入れない。**manifest の変更は WebAPK に焼かれるので、実機では入れ直さないと反映されない**（`chrome://webapks` の Package name が変われば新規インストール、Version code / Last Update Completion Time で更新の有無が分かる）。
- **上流の報告: https://issues.chromium.org/issues/561685220** （`Web Share Target on Android drops shared files from installed PWAs; text still arrives` / component `Blink>WebShare` / M153 のブランチ `branch-heads/8010` を参照）。**直ったら畳めるのは診断コードだけで、バナーは残す** —— 失敗の経路は Chrome の不具合以外にもある（`empty` / `stale` / `load:`）。
- **結論: Chrome 153 側の不具合**。共有シートから yomikake は起動するが、Chrome が multipart の終端行だけを送ってくる（＝パートを作る前にファイルを落としている）。共有元アプリ 3 種・端末 2 台・PWA 入れ直し・accept 変更のいずれでも変わらない。**Web 側に回避手段は無い**（Android Chrome には File Handling API も無い）ので、失敗時の動線（トースト → タップ → ピッカー）を実用品質に保つのが対策。
- **実機の実測（2026-09-19・Chrome 153・Android）: `nofile:absent/raw:nopart/keys=none/len=75/ct=mp`。** PWA の入れ直しでも別端末でも同じ。`len=75` は Chromium の 70 文字境界の終端行だけ＝**空の multipart**。 `formData()` は成功しているのに `epub` パートが無く、生 body にも `filename` 付きのパートが無い＝**Chrome が POST はするが ePub の実体を載せていない**。yomikake 側のコードは通っている（トースト → タップ → ピッカーで同じ本が開けることを実機で確認済み）。`keys=` / `len=` はこの次の切り分け（body ごと空なのか、空のパートだけ来ているのか）のために足した。
- **トーストは折り返す。** `#toast` は `white-space:nowrap` だったため、理由コードのような長い文が**画面の左右で切れて読めなかった**（実機のポートレートで発覚）。`max-width:min(560px, calc(100vw - 32px))` ＋ `white-space:normal` ＋ `overflow-wrap:anywhere` に変更（両ファイル）。短いトーストは中身なりの幅に縮むので見た目は変わらない。
- **各段に 20 秒のタイムアウト**（`shareTimeout`）。無応答は例外より質が悪く、実機では「固まった」としか分からない。
- **退避先は IDB → Cache Storage の二段**。`shareStash()` が IDB に失敗したら `SHARE_CACHE`（`yomikake-share-stash`）へ `Response` として置く。実装系統が別なので、容量やプライベートモードで片方が死んでも通ることがある。**`sw.js` の定数とページの `_SHARE_CACHE` / `_SHARE_STASH_PATH` は必ず対で直す**（`tests/cases/share-receive.js` が一致を検査する）。
- **⚠ `activate` の「VERSION 以外を消す」から退避キャッシュを除外してある。** 除外を外すと、共有した直後に SW が更新されただけでファイルが消える。
...
- **ヘルプ（`help.body`）にも節を置く。** 設定パネルにしか無いと機能の存在に気づけない。**しおりの節の後・読みかけリストの節の前**（設定パネルの並びと揃える）。中継の設置が要ること・`%` 表示が KOReader と一致しないこと・資格情報が書き出しに含まれないことの 3 点は必ず書く。`tests/cases/kosync.js` が 4 言語ぶん検査している。
...
- **覚えるのは 7 つだけ**: `writingMode` / `fontMode` / `fontSize` / `spreadMode` / `fxlRegionOrder` / `blankLines` / `dialogueGap`（後ろ2つは v2.25.0）。線引きの基準は「同じ本で前と同じであってほしいか」ではなく **「本が変われば変わるべきか」** — テーマ・明るさ・余白・行間・字間・タップ設定は読者の目や環境で決まるので対象外。
...
**空行の詰め方（v2.25.0）** (`markBlankBlocks`, `BLANK_BLOCK_SEL`, `BLANK_KEEP_SEL`, `BLANK_WS_RE`, `DIALOGUE_OPEN`, `changeBlankLines`, `toggleDialogueGap`, `updateDialogueGapToggleUI`, `_blankLeafBlocks`, `_isBlankBlock`, `measureBlankStyle`, `decideBlankAuto`, `applyBlankAuto`, `showBlankAutoToast`, `linesPerScreen`, `effBlankLines`/`effDialogueGap`, `_savedBlankLines`, `_blankAuto`, `state.blankLines`/`dialogueGap` — 設計書 `design_blank_lines.md`）。

- **Web 小説は投稿サイト側の組版機能が乏しいぶんを空行で代用している**ので、狭い画面では1画面の半分以上が空白になる。実測（蔵書20冊）で**紙の本・新書は空行率 2〜26%、Web 装飾は 43〜67%** と分布がはっきり割れる。空行の表現は本によって違う（`<p class="body-blank">&#160;</p>` / 素の `<p>` に空白1文字 / `<p><br/></p>`）ので**クラス名に依存した実装はできない**。
- **3 段階**（`off` / `collapse`＝連続空行を1行に / `all`＝すべて詰める）。**2 段階では足りない** —— 「連続を1行に」は**1行アキ型（薬屋のひとりごと・僕は異世界…）に一切効かない**（実測 98%）。逆に `all` だけだと紙の本の場面転換アキまで消える。効き方の実測は設計書 §1-2。
- **⚠ 空行の `<p>` を削除してはいけない。隠すだけにする。** kosync の XPointer は「同名兄弟の N 番目」で位置を指すので（`koNthChildByTag`）、段落を消すと添字がずれて **KOReader から受け取った位置が黙って別の段落に着地する**。目次・検索のアンカー（`#id`）も同じ理由で残る。`markBlankBlocks()` はクラス（`yk-bl` / `yk-bl-x` / `yk-gap`）を付けるだけで、消すのは `buildSrcdoc()` が注入する CSS の仕事。段階の切替も CSS の差し替えだけで済む。
- **`id` を持つ空行は詰めない** —— 目次・検索・kosync の着地点になりうるうえ、`display:none` の要素は `scrollIntoView` が効かず**黙って章頭に落ちる**。実測5冊で id 付きの空行は 0 件なので、この安全弁のコストは無い。
- **走査するのは「いちばん内側のブロック」だけ**（`el.querySelector(BLANK_BLOCK_SEL)` が無いもの）。章全体を包む `div` を数えると「中身が全部空行の div」まで空行になり、連続の数え方も狂う。
- **会話の半行アキは「境目」にだけ入れる**（`yk-gap`）。前後の両方に入れると**連続する会話の間にも空きが入り、詰めたのに間延びする**（紙の組版でも会話の連続は詰める）。**`margin` ではなく `padding`** —— 隣接ブロックのマージンは相殺されるので、ePub 側が `margin-block-end` を持っていると指定しても何も起きないことがある。量は `state.lineHeight × 0.5` em を描画時に焼く（`lh` 単位は古い iOS Safari に無い）。
- **行頭文字の判定で `indexOf('')` に注意** —— 空文字は 0 を返すので、`!!head &&` のガードが無いと**中身の無いブロックが全部「会話」になる**。
- **会話の半行アキは新しい本では必ず OFF**（`applyBookPrefs` の先頭で落とし、その本に明示的な記録があるときだけ復元）。ほかの項目と違って前の本の設定を持ち越すと**組版済みの紙の本の組版を黙って変えてしまう**。「空行の詰め方」のほうは持ち越す（紙の本では効果が 1〜2% しかなく無害）。
- 既定は `off` / 会話アキ OFF（＝走査ごと行わない）。**本ごとに記憶する**（Web 小説か紙の本かは本の属性）。`change*()` はグローバルと本ごとの両方に書く既存の作法に従う。読み上げ・受け渡し・全文検索・しおり書き出しは**生の ePub を読む**ので無影響。
- **おまかせ（`auto`・v2.25.0）** — 本を開くときに本文のある章を最大12章サンプリング（本文300ブロックで打ち切り）して3つの数を測り、自動で決める（`measureBlankStyle` → `decideBlankAuto` → `applyBlankAuto`）。**既定は `off` のままで、選んだ人にだけ効く。**
  - **判定の根拠は実測**（蔵書13冊）: **空行/本文比が紙・新書 0.00〜0.26、Web 装飾 0.62〜1.91 と2倍以上離れている**ので 0.5 で切れば誤爆しない。**「地の文どうしの間に空行があるか」が「意味を持つ空行」かどうかを分ける** —— １０歳から始める冒険者生活は会話の前 100% / 地の文どうし 4%＝空行が会話の区切りという意味を持つ本で、ここだけ詰めたあとに半行アキで呼吸を戻す。
  - **狭い画面＝すべて詰める / 広い画面＝連続を1行に。判定は px ではなく「1画面に入る行数」**（`linesPerScreen()`＝読書エリアの長さ ÷ 文字サイズ×行間・境界25行）。**文字を大きくしている人は PC でも狭い扱い**になり、実際の読みにくさに連動する。
  - **半行アキは「すべて詰める」のときだけ** —— 「連続を1行に」では会話まわりの空行が1行残っており、そこへ半行を足すと二重になる。
  - **測る側と描く側は同じ `_blankLeafBlocks()` / `_isBlankBlock()` を使う**（見方がずれると「測った本と描く本が違う」ことになる）。
  - **⚠ おまかせ中に手で選び直したら、その本だけ固定し、グローバルは `auto` のまま保つ**。印は `_blankPinned` で持ち、**`saveSettings()` が `blankLines` だけ `'auto'` を書き戻す** —— `changeBlankLines` の中だけで済ませると、**固定中にテーマなど他の設定を触っただけで方針が消える**（実装中に踏んだ）。**本を開いていないとき（読みかけリスト）の変更は方針そのものの変更**として扱う（固定する先の本が無いため）。1冊の上書きでライブラリ全体の方針が解除されるのは意図と違う ——「最後に使った設定が新しい本の既定になる」という既存の作法への**ここだけの例外**。`applyBookPrefs` が毎回 `state.blankLines` を**保存値**へ戻すので、固定した本の値が次の本へ漏れない。
  - **黙って見た目を変えない** —— 詰めたときは**その本で一度だけ**トースト告知（`showBlankAutoToast`・`blankAutoSeen` を `_bpSet`）、タップでその本だけ「そのまま」に戻る。戻した本は、開いたまま設定で**「おまかせ」を選び直せば再判定**される（本ごとの記録も `auto` に戻る）。
  - **⚠ `changeBlankLines()` はセレクトの値も同期する** —— この関数は告知トーストからも呼ばれるので、同期しないと**「設定欄はおまかせのまま・中身は固定」という嘘の状態**になる。セレクトが既に `auto` を表示していると**同じ値を選んでも change イベントが飛ばず、おまかせに戻す手段が消える**（実機の導線を追って判明）。
  - **⚠ `linesPerScreen()` は `#page-container` を直接測ってはいけない** —— `applyBlankAuto()` が走る時点では `.visible` が付く前で **`display:none` の 0×0** を返す。0 を採ると必ず「狭い画面」と判定され、**広い画面の `collapse` 分岐が死ぬ**（1600px の窓でも「すべて詰める」になっていた・コードレビューで発覚し実測で確認）。実寸を持つ親（`#reading-area` → `#main` → window）へ落とすこと。
  - **⚠ 状態を変えたら必ず `syncBlankUI()` を通す**（セレクト＋トグル＋注記）。`applyBookPrefs` の**早期 return 2 箇所**もここを通す —— state だけ変えてセレクトを置き去りにすると、前の本の値が残り、**同じ値を選んでも change が飛ばないので直せない**。
  - **⚠ `toggleDialogueGap()` は本を開いているときだけ固定する** —— 本未オープンで `state.blankLines` を書き換えると、直後の `saveSettings()` が**ライブラリ全体の「おまかせ」を黙って潰す**。
  - **⚠ `resetDisplaySettings()` は `_blankPinned` / `_blankAuto` も落とす** —— 残すと `saveSettings()` が `blankLines` に `'auto'` を書き戻し、**リセットしたのにここだけ戻らない**。
  - **⚠ `_isBlankBlock()` は子孫まで見る** —— `<p><a id="note1"></a></p>`（脚注の戻り先）は `textContent` が空なので、自分の `id` しか見ないと隠れて**目次・脚注のジャンプが黙って章頭に落ちる**。`[id],a[name]` を持つものは空行にしない。**`li` / `td` / `th` は空でも絶対に詰めない**（`display:none` にすると `<ol>` の番号が繰り上がり、表の行から消えて以降の列がずれる）。
  - **設定パネルに現在の状態を1行出す**（`updateBlankNoteUI` / `#blank-auto-note`）—— おまかせは結果が見えないと理解できない。`data-i18n` を持たない動的テキストなので `applyI18n()` の末尾からも呼ぶ。
  - FXL では測らない。本文ブロックが150に届かない本（短編・詩集）は**判定しない**＝そのまま。
- テストは `tests/cases/blank-lines.js`（両ファイル各 65 assertion・fixture `tests/.fixtures/blank.epub` と `blankweb.epub`）。**`run.sh` で `--window-size=520,900` を固定してある** —— おまかせの判定は「1画面に入る行数」を見るので、窓幅が境界（25行）を跨ぐとフレークする。**実際に本を開いて iframe の中の実寸を測る**（クラスの有無だけを見ると「クラスは正しいのに表示が変わっていない」を取り逃がす）。⚠ **`tests/lib/dom-test.sh` の出力先 `#yomi-test-out` は画面外に固定してある** —— 本文の流れに置くと結果が増えるたびにページが伸びてアプリの読書エリアが縮み、**同じ章が assertion の数だけ長くなって見える**（実寸比較が成立しない。この機能の開発中に踏んだ）。
...
- **章末判定の丸め誤差許容（`EDGE_EPS = 4`・両ファイル・3 モードすべて）** — 表示倍率が整数でない環境（Windows の 125%/150% 拡大表示、ブラウザのズーム）では `100vw` が小数になり、**切り上げの `scrollWidth` と切り捨ての `clientWidth`** の差で `sw - 2*vw` に **1px の正の誤差**が残る（実測: dsf 1.1–2.5 で最大 1px）。本文がちょうど 1 画面ぶんの**全画面画像ページ**（`img{width:100%;height:100%}` の表紙・挿絵）では、この 1px が「本文終端まであと 1px」という偽の余白ゾーンを作り、**次へを押すと章が変わらず真っ白なページが 1 枚挟まる**（もう一度押せば進む）。→ 各 IIFE の先頭に `var EDGE_EPS = 4;` を置き、`sw <= 2*vw` / `sh <= 2*vh`（iOS 版は `ms <= 0`）の判定と `applyInit` / `reportPos` の range 判定を**すべて** `+ EDGE_EPS` 側に倒す。畳んで失うのは 1 文字に満たない幅なので読み落としは起きない。
  - ⚠ **「Chrome だけで起きる」は engine の差ではなく窓幅の丸めの偶然**。同じ Chromium の Edge でも Firefox でも窓幅とズーム次第で起きる。**ページ余白の設定で再現が変わるのも同じ理由** — 「標準」(`max-width:900px`) と「中」(`1400px`) は iframe 幅が**整数で止まる**ので構造的に誤差が入らず、「狭」(`2000px`) と「なし」(`100%`) だけが窓幅そのものに依存する。ブラウザ・窓幅・倍率ごとの実測は `tests/probe/scroll-rounding.html`（開くだけで 4 つの余白設定の「余り」が出る）。
  - テストは `tests/cases/scroll-edge.js`（両ファイル各 10 assertion）。`padding:calc(100vw + 1px)` で誤差を焼いた iframe に**実際の注入コードを流して** `EPUB_EDGE` が返ることを見る。⚠ 併せて「本文が複数画面あるときは畳まずスクロールする」も検査している —— 許容値を広げすぎる改修への歯止めなので消さないこと。
...
- **`INIT_FN` timing (iPad fix)** — on iPad, `body.offsetWidth` is read before `writing-mode:vertical-rl` layout completes, causing `maxS()=0` and content positioned off-screen. The fix uses a double-rAF (fast path for iPhone) plus a `setTimeout(runApplyInit, 500)` fallback (ensures layout is complete on iPad). A `_initApplied` flag prevents the second call from resetting the reading position if the first already succeeded. `runApplyInit` also runs the visibility:hidden → layout flush (tcy fix) and fires `EPUB_READY`:
  ...
  ```
- **`reflowKeepPos()` — viewport 寸法が変わったら位置を引き直す（v2.23.0）。** transform スクロールでは位置（`tx`/`ty`）が **viewport 寸法に依存する**（`ms = body.offsetWidth - vw()`）。サイドバーの開閉・全画面の切替・回転で寸法が変わると、**同じ `tx` が別の位置を指す**。初期から iframe 側に `resize` ハンドラが無く、**目次から章へ飛んだあとサイドバーを閉じると、本文が左にずれて右にサイドバー幅ぶんの余白が出ていた**（縦書きは右端が読み始めなので、`vw` が広がると `ms` が縮み `tx` を `+Δ` 右へ動かす必要がある）。
  - 直近の読書位置を**比率**で `_ratio` に持ち（更新点は `setTx`/`setTy` の中だけ）、`resize` 後 150ms のデバウンスで `reflowKeepPos()` が新しい `ms` から引き直す。寸法が変わっていなければ何もしない（`_rzW`/`_rzH`）。`_snapOrigin` は再計測させる。
  - **3 つの IIFE すべてに要る**（縦書き＝`tx`・横書き＝`ty`・publisher＝軸で分岐）。**比率の向きが軸で逆**（縦書きは `0=先頭` が `tx=-ms`、横書きは `0=先頭` が `ty=0`）なので、コピペで済ませないこと。
  - **`yomikake.html` には入れない。** あちらは本物のスクロール API なのでブラウザが位置を保つ。transform スクロールは iOS 専用の機構であり、この補正もそれに属する。`tests/cases/scroll-reflow.js` は**本体側に入っていないこと**も検査している。
  - ⚠ `tests/lib/syntax-check.js` は HTML 内の `<script>` しか見ず、**iframe へ注入される文字列は素通りする**。`scroll-reflow.js` が 3 モードとも `new Function(src)` で構文を確かめている。
...
- **起動時のアカウント選択をなくす（設計書 `design_drive_silent_auth.md`・両ファイル）** (`DRIVE_ACCOUNT_KEY`, `_driveAccountGet`/`_driveAccountSet`/`_driveAccountForget`, `_driveRememberAccount`, `_driveAuthedOnce`, `_authOnToken`/`_authOnError`, `_isDrivePopupError`, `_driveGestureRetry`, `driveRetryOnGesture`, `DRIVE_RETRY_SKIP_SEL`, `_driveAcctGen`)
  ...
  - **⚠ GIS は小窓を開けなかったとき `error_callback` を同期で呼ぶ**（`requestAccessToken` が戻る前。iOS Safari の実機記録で特定）。つまり `new Promise` の executor の中で決着しうるので、**`_authPromise = new Promise(...)` と書いてはいけない** —— 決着後に代入され、失敗済みの Promise が「進行中の認証」として残って以後の `driveAuth()` が全部その失敗を返す（取り直しも「認証中」と見て見送る）。`settled` を見てから代入する。GIS の例外も executor 内の try/catch で `settled` を立てて reject。**テストのモックは同期で返すモード（`failopen-sync`）も持つこと** —— 非同期でしか返さないモックがこの不具合を隠していた。
  ...
  - **iOS Safari は（タブで開いた場合）ユーザー操作なしの起動直後に必ず `popup_failed_to_open`**（iPad・iPhone・全条件で probe 実測。PC でもタブが裏で開くと出る）。**ホーム画面から起動した場合は起動直後に通る**（実機確認済み・タップ不要で読書が始まる）。このときだけ `_driveGestureRetry` を立て、**次の親 `click`（capture）または `EPUB_TAP` の受信**で `driveRetryOnGesture()` が取り直す。**⚠ ユーザー操作の権利が要る操作（`DRIVE_RETRY_SKIP_SEL`＝ファイルピッカー・全画面・読み上げ開始・共有・入力欄・リンク・カード・設定パネル・モーダル）のクリックとキー操作では取り直さない** —— 小窓を開くとそのクリックの権利を使い切り、同じ操作の後続が弾かれうる。**⚠ ただしボタン全般を除外してはいけない** —— iOS 版のページ送りはスワイプ（権利なし）かボタンなので、読書中に取り直す機会が消え、「リストへ」（`#open-btn.reading`）まで除外すると閉じるときの保存が失敗する（実機で踏んだ）。「リストへ」で取り直せば `driveSaveNow` の `driveAuth()` が進行中の認証を待って保存できる。**`[tabindex="-1"]` は除外に入れない**（`reclaimKeyFocus()` が `#page-container` に付けるので FXL のタップまで弾く）。iframe 内のタップはユーザー操作として親にも及ぶので postMessage 受信時点でも開ける（実測）。**⚠ `touchend` は使わない**（iOS ではまだユーザー操作と認められない・実測）。**⚠ ハンドラから `requestAccessToken` まで `await` を挟まない**（文脈が切れる）。`popup_closed` では仕掛けない（閉じたのは利用者の意思）。
  - **⚠ 自動保存の失敗時は最低 `AUTO_SAVE_RETRY_MIN`（30 秒）空ける。** 旧実装は「前回の成功から 1 分」で再試行を組んでいたので、**一度も成功していない（`_lastAutoSaveAt=0`）と待ち時間が 0 になり、失敗→即再試行が数ミリ秒間隔で止まらなかった**（iOS の Safari タブで 8ms 間隔を実測。通信断でも同じ＝v2.25.0 以前からの潜在不具合。認証が固まっていた頃はそこで止まっていたので表に出なかった）。認証画面を開けなかった失敗（`waitGesture`）では再試行を組まず、`scheduleAutoSave` も `_driveGestureRetry && !_driveToken` の間は送りに行かない。取れたら `driveRetryOnGesture` が保留分を送る。
  - ポップアップ由来の失敗（`_isDrivePopupError`）では**自動保存を止めない**（止めると iOS の起動直後や、うっかり閉じただけで自動同期が OFF になる）。
  - **自動同期 OFF で記憶とメモリ上のトークンを捨てる**（`_driveAccountForget`）。about の応答待ちの間に忘れられた分は `_driveAcctGen` で書き戻さない。ON に戻すとアカウント選択から始まる＝アカウントを切り替える唯一の手段。回復不能で止めたときも同じ。
  - 実測の検査ページは `tests/probe/drive-silent-auth.html`（OAuth の承認済み生成元 `https://www.ayati.com` 配下でないと動かない）。テストは `tests/cases/drive-auth.js`（両ファイル各 70 assertion・GIS をモックして requestAccessToken の引数・固まらないこと・click で**同期的に**呼ばれることを見る）。**ポップアップが実際に出るか・チラつき・iOS でブロックされるかは実機でしか確かめられない。**
