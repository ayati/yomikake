...
- **実機の実測（2026-09-19・Chrome 153・Android）: `nofile:absent/raw:nopart`。** `formData()` は成功しているのに `epub` パートが無く、生 body にも `filename` 付きのパートが無い＝**Chrome が POST はするが ePub の実体を載せていない**。yomikake 側のコードは通っている（トースト → タップ → ピッカーで同じ本が開けることを実機で確認済み）。`keys=` / `len=` はこの次の切り分け（body ごと空なのか、空のパートだけ来ているのか）のために足した。
