...
- **`detectAxis()` in publisher mode** — publisher mode cannot know writing-mode at CSS injection time, so `buildSrcdoc()` injects `body { height:max-content; width:max-content }` as a neutral starting point. `detectAxis()` (called from `applyInit()`) reads `getComputedStyle(body).writingMode`, determines axis, then dynamically sets the correct constraints. For 'h' axis (vertical-rl): adds `body { bottom:0; height:auto }` — critically, `height:auto!important` via inline style overrides the stylesheet `height:max-content!important` (inline `!important` wins over author stylesheet `!important` per CSS cascade), allowing `top:0; bottom:0` to constrain body height to 100vh and force column wrapping. For 'v' axis (horizontal-tb): adds `body { right:0; width:auto }` similarly to constrain body width to 100vw. Without `height:auto` for 'h' axis, `height:max-content` wins and body grows to one tall column with no wrapping.
