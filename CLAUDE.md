...
- **必ず理由コードを返す。** `sw.js` は `?shared=err&r=<理由>` で返し（`form:<例外名>` / `nofile:empty` / `nofile:absent` / `raw:nopart` / `idb:<名>/cache:<名>` / `*:TimeoutError`）、ページ側は `empty`（退避に何も無い）/ `stale`（鮮度切れ）/ `load:<例外名>` を足してトーストに出す。**「原因不明のまま握りつぶす」を新たに作らないこと。**
