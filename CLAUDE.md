...
Each viewer is a single self-contained HTML file (`epub_viewer.html` ~2074 lines, `epub_viewer_ios.html` ~2126 lines). Both follow a modular functional style with a single central state object. The architecture below describes `epub_viewer.html`; `epub_viewer_ios.html` is identical except for the scroll mechanism (see iOS Viewer section below).
...
- **Android touch device visibility (`@media (hover:none) and (pointer:coarse)`)** — `.chapter-btn` is shown at `opacity:.3` (with `stroke:var(--ui-text)`) and `#btn-scroll-fwd` at `opacity:.22` on touch devices, matching the pattern used in `epub_viewer_ios.html`. The narrow-screen rule (`@media (max-width:640px)`) does **not** apply `background:var(--accent)` to `#btn-scroll-fwd`. If it did, removing the `nav-hint` class would leave `#btn-scroll-fwd` visually unchanged (permanent accent color ≡ flash color), making the button appear to stay after the flash ended — the Android-only bug. The `#btn-scroll-fwd { opacity:.22 }` rule wins over `.scroll-btn { opacity:.9 }` in the narrow-screen rule because ID selector specificity beats class selector specificity.
