...
### 実機で見つかった UI の不具合（2 件・修正済み）

| 症状 | 原因 | 修正 |
|------|------|------|
| 狭幅 Android でプレイヤーバーから 📤 がはみ出して切れ、初見で操作が分からない | `#tts-bar` の中身は `flex:none` / `min-width` 持ちで縮まないため、`max-width` を超えると**ピルの外へ溢れて描画される** | `flex-wrap:wrap` を追加（どの幅でも溢れない構造的保証）＋ `@media(max-width:640px)` で 1 段に収まるよう詰める |
| 設定パネルから開くと受け渡しダイアログが見えない | `#settings-popover` は `z-index:500`、`#modal-overlay` は `200`。**モーダルが設定の後ろに描画される** | `showTtsHandoff()` の冒頭で設定パネルを閉じる |

2 件目は modal 側の z-index を上げる手もあるが、`loading:250` / `toast:300` まで押し上げる必要があり
波及が大きい。「モーダルと設定パネルは排他」という既存の前提を守る側で直した。
**今後 `#modal-overlay` を設定パネルから開く導線を足すときは同じ処理が要る。**

> ⚠ **1 件目は自動テストで症状を再現できない。** headless Chrome はウィンドウを 500px 未満に
> 縮められず（`--window-size` を渡しても `innerWidth` は 500 で頭打ち）、実機の不具合は
> 343px 相当で起きたため。そこでテストは幾何ではなく **`flex-wrap:wrap` という仕組みそのもの**を
> 検査している。両修正とも「戻すとテストが落ちる」ことを確認済み。
> CLAUDE.md の「担保できないこと: 500px 未満の実寸レイアウト」に該当する事例。
