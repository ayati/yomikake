...
| 版数 | 項目 | 設計書 |
|------|------|--------|
| **v2.15.0** | B-11 行間の連続値化＋字間 / B-12 読み上げ速度を設定にも / B-13 画面の向きロック | `design_display_settings_v3.md` |
| **v2.16.0** | B-7 本ごとの表示設定 | `design_per_book_settings.md` |

B-7 だけ分離したのは、唯一**新しい永続データ構造**（`epub_book_prefs`）を持ち、
「どの設定を本ごとにするか」の線引きとリセットとの関係を独立して詰める必要があるため。

不採用のまま: **B-8 表示設定の同期**（端末ごとに画面・DPI・フォント資産・OS が違うため
表示設定は端末ローカルに閉じる）、**B-10 カスタムテーマ**（先送り）。
