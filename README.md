...
- **コマ読みズーム（FXL 専用）**：1 ページを 2列×3行=6領域に分割し、Next ボタン連打 / ダブルタップ / ドラッグ PAN / ラバーバンドで快適にコマ単位の読書が可能。領域遷移順は「ストーリー」「4コマ」を選択式、LTR 書籍は自動で左右反転
...
### コマ読みズーム（FXL 専用 / Phase 3）

マンガ・雑誌で 1 ページを小画面上で読む際、1 ページを **2列 × 3行 = 6領域** に分割し、Next ボタン連打で自動的に次の領域へズームしながら読み進められる機能です。5 タップで 1 ページ読了、6 タップ目で次ページの 1 領域目に入ります。

**起動方法**

- ツールバーの **🔍 ズームボタン**（FXL 本を開いている間だけ表示）
- 画面を **ダブルタップ**（タップした位置の領域にそのまま寄ります）
- キーボード **`z`**（デスクトップのみ）
...
| 種類 | 遷移順 | 用途 |
|------|--------|------|
| **ストーリー** | ①右上 → ②左上 → ③右中 → ④左中 → ⑤右下 → ⑥左下 | 通常のマンガ・見開きコマ |
| **4コマ** | ①右上 → ②右中 → ③右下 → ④左上 → ⑤左中 → ⑥左下 | 縦 4 コマレイアウトのマンガ |

LTR 書籍（洋書マンガ・英文雑誌など `page-progression-direction="ltr"`）は自動で左右反転されます。⚙設定の「**LTR書籍で反転**」を OFF にすると反転を無効化できます。

**操作系**

- **Next ボタン（次へ↓）**：ズーム中は「次の領域」、最終領域のときは「次ページの 1 領域目」
- **Back ボタン（↑前へ）**：対称に「前の領域」または「前ページの 6 領域目」
- ボタンの端に **バッジ**（`2/6`, `⏭` など）が表示され、次タップでの動作が一目で分かります
- **ドラッグ**：ページ上で指やマウスをドラッグすると **自由 PAN モード（🌀）** に移行。プリセット位置に縛られず自由に画面内を移動できます
- **領域ピル**（右下の丸いボタン）：
  - `🎯 3/6` 表示（プリセット中）：タップで次領域へ（Next と同機能）
  - `🌀 自由` 表示（自由 PAN 中）：タップで元の領域（直前の regionIdx）にスナップ復帰
- **ラバーバンド**：ズーム中に画面端からさらに読み進み方向へ 50px 以上引っ張ると、次／前ページに一気にジャンプしつつ次ページの 1 領域目に自動でズームします
...
| 項目 | 選択肢 | 既定値 |
|------|--------|--------|
| **拡大倍率** | 1.5x / 2.0x / 2.5x | 2.0x |
| **領域順** | ストーリー / 4コマ | ストーリー |
| **LTR書籍で反転** | ON / OFF | ON |

**見開き時の挙動**：見開き表示中も 6 領域は「片側ページ単位」で定義されます（= 見開き全体では 12 領域）。現在フォーカス中のページ（`state.currentSpineIdx` が指す側）の 6 領域を順次たどり、6 領域目の Next でもう一方のページに自動切替、そのページの 6 領域終了で次ペアへ進みます。これにより見開きレイアウトを崩さず、特定のコマやセリフに集中できます。

**永続化ポリシー**：ズームの ON/OFF 状態・現在の領域位置・PAN オフセットは **localStorage には保存しません**（本を開くたびにズーム OFF で起動）。倍率・領域順・LTR 反転の 3 つの設定のみ `epub_settings` に永続化されます。
...
| **拡大倍率**（コマ読みズーム） | 1.5x / 2.0x / 2.5x | FXL |
| **領域順**（コマ読みズーム） | ストーリー / 4コマ | FXL |
| **LTR書籍で反転**（コマ読みズーム） | ON / OFF | FXL |
