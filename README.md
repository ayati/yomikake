...
- フォント・文字サイズ・行間・テーマ・余白・**次へボタンのサイズ**のカスタマイズ（設定は自動保存）
...
- **Google Drive しおり自動保存**：設定パネルでオンにすると、読み進めるたびに自動でDriveに保存（最小間隔1分）
...
- トークン期限切れ（約1時間）を検出すると自動でオフになり、トーストで通知します
...
| Drive OAuth トークン管理 | `driveAuth()` | アクセストークンはメモリのみ保持（localStorage 非使用）、期限切れ時に自動クリア |
| Drive ファイルID バリデーション | `driveFindFile()` | API レスポンスの fileID を `/^[a-zA-Z0-9_-]+$/` で検証してから URL に使用 |
