...
- **読み上げ（音声のみ・v2.9.0）**：ツールバーの 🔊 ボタンで、開いている本文を端末の音声で読み上げます（リフロー型のみ）。画面下部のプレイヤーで 前の文／一時停止・再開／次の文／停止・速度（0.5〜2.0倍）を操作でき、章末では自動で次章へ進みます。**ルビ（ふりがな）がある箇所はふりがなを読む**ため、なろう系 Web 小説の人名・固有名詞も正しく発音されます。音声の種類は本の言語で絞り込んで選択でき、再生中は画面が消灯しないよう Wake Lock で保持します。ブラウザの制約により画面ロック・タブ切替では停止します（Web Speech API・追加ダウンロード不要・オフライン可）
...
| `epub_settings` | フォント・文字サイズ・行間・テーマ・余白・組方向・次へボタンサイズ・**タップページ送り**・**起動時に前回の本を開く**・**読み上げ速度/音声**・**見開き表示**（FXL用）・**太字表示**・**縁取りの強さ** |
