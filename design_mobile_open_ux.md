...
## Phase 2 追補（2026-09）: 受け取りの失敗を追えるようにする

**症状**: Android 実機（Chrome 153・インストール済み PWA）でファイラーからの共有が
`toast.shareFailed` で失敗する。

**調べたこと**:

- 共有経路のコードは v2.6.0 から 1 行も変わっていない（`git log -S"_shareIdbTake"`）。
  v2.21.0→v2.23.0 の差分にも share 関連は無い。
- 手元（Chrome 145・localhost・実 Service Worker）で、実際に失敗している 2 冊
  （411KB / 6.3MB・`～`・`【✰２０００突破！】` を含む実ファイル名）を
  **本物のナビゲーション POST** で流したところ、SW 受信 → 退避 → ページが拾って
  **本が開くところまで完走**した。サイズでもファイル名でも ZIP の中身でもない。
- したがって原因は端末側（Chrome 153 / content:// / ストレージ事情）だが、
  **失敗点が 3 箇所あって全部が同じ文言を出すため、実機から情報を持ち帰れない**。
  これが直すべき本体。

**入れたもの**:

1. **理由コード** — `sw.js` は `?shared=err&r=<理由>`、ページは `empty` / `stale` /
   `load:<例外名>` を足してトーストに出す。
2. **各段に 20 秒のタイムアウト** — 無応答を例外に変えて見えるようにする。
3. **退避先の二段化** — IDB が駄目なら Cache Storage。
4. **`formData()` のフォールバック** — 生の body から epub パートを自前で切り出す。
5. **覗くだけで消さない** — 削除は `loadEpub` が通ってから。失敗した共有を再試行できる。
6. **失敗時の逃げ道** — 「タップでもう一度」／「タップしてファイルを選ぶ」。
...
### 2 回目の実機（2026-09-19 12:12〜12:16）

**`nofile:absent/raw:nopart/keys=none/len=75/ct=mp`** — しかも
**ファイラー / Google Drive / Files by Google の 3 つとも同一**。

- `keys=none` … フォームにパートが 1 つも無い（テキストすら来ていない）
- `len=75` … **body は空ではない**。multipart の枠だけが 75 バイト届いている
- 3 つの共有元で同じ → **共有元アプリは無関係**。Chrome 側の問題

つまり Chrome は「共有ターゲットへ POST する」ところまではやっていて、
**ファイルを body に載せていない**。Chrome が accept に照らしてファイルを落としたか、
content:// を読めずに空の multipart を送ったか、のどちらか。

次の 1 回で 75 バイトの中身（`body=`）を見る。

**⚠ Android の共有ターゲットは WebAPK のインテントフィルタで OS に登録される。**
共有シートに出るかどうかは**インストール時に焼かれた WebAPK** が決め、実際の POST は
...
対応は「直せないものを直そうとしない」方針で 2 つ:

- **A. 失敗時の動線を格上げ** — `#share-fail`（読みかけリストの先頭に出る消えないバナー）。
  「ファイルを選ぶ」ボタンでピッカーへ（クリック由来なのでユーザージェスチャがある）。
  実体が手元にある場合だけ「もう一度開く」に変わる。読書中は `#welcome` が隠れているので
  トーストに落とす（`reportShareFailure` が `state.epub` で振り分ける）。
- **B. 文面を普段使いに戻す** — 本文から理由コードを外し、バナーの小さい行（10px・淡色・
  等幅）と `console.warn` に残す。**診断を消してはいけない**。次に別の壊れ方をしたとき、
  実機から情報を持ち帰れる唯一の手段になる。

Chrome が直れば共有はそのまま動く（こちらのコードは何も変えなくてよい）。
