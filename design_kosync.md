...
**実装状況: Step 0〜5 実装完了・実機確認済み（2026-08-30）。リリース待ち。**
テストは `tests/cases/kosync.js`（両ファイル各 211 assertion）。全体 1845 PASS / 0 FAIL。
ドキュメントは `README.md`（利用者向け）と `CLAUDE.md`（設計メモ）に反映済み。
...
**生成したパスは自分で検算してから送る** — 作った XPointer を §4-2 の解決器へ流し、
**元の要素に戻ってくることを確認する**。戻らなければ**その要素の親で組み直して**試し、
それでも駄目なら章頭 `/body/DocFragment[N]/body` に落とす。この形は必ず解決できる。

**どの要素を指すかは `_intraChapterRatio` から決める。** iframe の中を直接測りに行くと
`yomikake_ios.html` の transform スクロールぶんだけ別実装が要るので触らない。
章の XHTML を読み、**テキストを持ついちばん内側のブロック要素**を文書順に集めて
**文字数で按分**する（スクロール量は文字量にほぼ比例する）。この作りなら
`ruby` / `rt` / `span` は候補に入らないので、「ブロック要素どまり」が**構造的に保証される**。
...
### 7-3. ~~押し戻す XPointer が crengine で解決される保証は無い~~ → 実機で解決（2026-08-30）

§2-5 で確かめられたのは「crengine → ブラウザ」の向きだけで、**逆方向は実機でしか
確かめようがなかった**。§4-3 の **(a) ブロック要素どまり (b) 送る前に自分で検算
(c) 駄目なら章頭へ落とす** の 3 段で当てにいき、**実機で通った**（§8-1）。

3 段の守りは残す。ePub の作りは千差万別で、(c) に落ちる本はいずれ出る。
落ちた場合は長い章で位置が戻るので、その可能性は説明文に残しておく。
...
| **5** | 自動同期（S5 の安全弁込み） | **✅ 実装完了（2026-08-30）** — 安全弁は `window.fetch` を差し替えて検証。実通信は**デプロイ後に実機で**（§8-1）|
...
### 8-1. 実機確認 — ✅ 完了（2026-08-30）

**リフロー本・FXL 本（AKIRA1）とも、取得・送信の双方向で同期成功。
`yomikake_ios.html`（iPad）でも双方向で確認済み。** これにより、自動テストで担保できなかった
2 点がどちらも決着した:

1. **接続まわり** — Apache の `/kosync/` 中継を通した実通信が成立する
2. **push した XPointer を crengine が解決できる** ★本命 —— §2-5 で確かめたのは
   「crengine → ブラウザ」の向きだけで、**逆向きは実機でしか確かめようがなかった**。
   §4-3 の 3 段（ブロック要素どまり・自己検算・章頭フォールバック）で当てにいった設計が
   実際に通ったことになる。

FXL が通ったことも設計どおり（§4-5。1 spine = 1 ページで無損失）。
**iOS 版も同じコードで通った**ことは、pull の着地を既存のアンカースクロール
（`'#id'` 経路）に載せた判断が正しかったことの裏づけでもある —— 位置計算を自前で
書いていたら、iOS の transform スクロール向けに別実装が要り、そこが独立した
バグ源になっていた（§4-2）。

**確認できた組み合わせ**:

| yomikake 側 | KOReader 側 | 本 | 向き |
|---|---|---|---|
| `yomikake.html`（PC） | Android / PocketBook | リフロー・FXL | 双方向 ✅ |
| `yomikake_ios.html`（iPad） | 同上 | — | 双方向 ✅ |

以下は当初の手順（再検証やトラブル時の参照用に残す）。

手順:

| # | やること | 期待 |
|---|---|---|
| 1 | ayati.com へデプロイし、設定に `https://www.ayati.com/kosync` ＋ 既存アカウントを入れる | 「接続テスト」で成功トースト |
| 2 | KOReader（Android か PocketBook）で本を途中まで読み、同期する | — |
| 3 | yomikake で**同じ ePub ファイル**を開き「⬇ KOReader から取得」 | 「◯◯ は N% まで読んでいます」→ タップで**その段落へ着地** |
| 4 | yomikake で別の場所まで読み「⬆ KOReader へ送信」 | 送信トースト |
| 5 | KOReader 側で同じ本を開き直す | **yomikake で読んでいた段落あたりが開く** ★ここが本命 |
| 6 | 同期方法を Filename に変えて 3〜5 を繰り返す | 同じ結果 |
| 7 | マンガ（AKIRA 等）で 3〜5 | ページがぴたりと一致する |
| 8 | 自動同期を ON にして、本を開く／読み進める | 開いたら提案が出る・1 分ごとに送られる |

**5 が外れた場合**でも、章頭までは必ず開くはず（`/body/DocFragment[N]/body` へ落ちる作りのため）。
「章は合っているが段落が違う」なら §4-3 の検算は通っていて crengine 側の構造が違うということなので、
そのときは KOReader が書いた `last_xpointer` と yomikake が送った文字列を突き合わせれば原因が分かる。

**残る未検証（急がないもの）**: 乱れた ePub での `autoBoxing` 実例（§8 Step 6）、
`percentage` の丸め桁数（判定に使わないので実害なし）、公式サーバ以外の実装差（§9-3）。
