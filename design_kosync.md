...
**実装状況: Step 0〜5 すべて実装完了（2026-08-30）。残るは実機確認のみ（§8-1）。**
...
### 8-1. 実機確認の手順（未実施・デプロイが要る）

自動テストで担保できないのは 2 つだけ。**どちらもデプロイしないと試せない**
（localhost には `/kosync/` が無く、設定欄に send2ereader を直接入れると CORS で弾かれる）。

1. **接続まわり** — 設定を入れて「接続テスト」が通ること
2. **push した XPointer を crengine が解決できるか** ★本命
